...
Also, MicroFun R package also provide `fun2tax_box()` to help user visualize from functional ID with annoted higher level functional description.

```r
p2 <- fun2tax_box(stratified_path = "~/Downloads/pred_metagenome_contrib (3).tsv.gz", physeq = ps1, group = "group", pathway_sdaa_result = result_fun,taxon_level = "Class", pathway = "KO)
p2[[1]]
```
